...
@register_model('fconv_attn')
class FConvAttnModel(FairseqModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        return FConvAttnModel(encoder, decoder)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, target_embedding, encoder_out, encoder_padding_mask):
        ...
        # change from encoder_out[1] to encoder_out[0]
        # which means the weighted context are encoder outputs only, no embeddings
        # now the attention keys are the same as attention values
        x = self.bmm(x, encoder_out[0].permute(0, 2, 1))
...
@register_model_architecture('fconv_attn', 'fconv_attn')
...
@register_model_architecture('fconv_attn', 'summ_vanilla_attn')
