...
@register_model('fconv_avg')
class FConvAVGModel(FairseqModel):
    """
    ...
    def build_model(cls, args, task):
        ...
        return FConvAVGModel(encoder, decoder)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, target_embedding, encoder_out, encoder_padding_mask, flag):
        residual = x
        ...
        sz = x.size()
        # if flag:
        #     x = x + torch.max(x).clone().detach()
        scale_ratio = 0.5
        x = x + scale_ratio * torch.max(x).clone().detach()
    ...
    def forward(self, prev_output_tokens, encoder_out_dict=None, incremental_state=None):
        ...
        for proj, conv, attention, res_layer in zip(self.projections, self.convolutions, self.attention,
            ...
            if attention is not None:
                ...
                x, attn_scores = attention(x, target_embedding, (encoder_a, encoder_b), encoder_padding_mask, flag)
...
@register_model_architecture('fconv_avg', 'fconv_avg')
...
@register_model_architecture('fconv_avg', 'summ_avg')
